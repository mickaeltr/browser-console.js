(function () {
    ...
    console.proxyLogFunction = function (level, originalOrDefaultFunction) {
        return function () {
            var message, returns;
            try {
    ...
    console.proxyLogFunctions = function () {
        ...
        for (index = 0; index < console.config.levels.length; ++index) {
            ...
            console[level] = console.proxyLogFunction(level, originalOrDefaultFunction);
        }
    ...
    console.handleJavaScriptErrorsLogging = function () {
        if (console.config.logJavaScriptErrors) {
            ...
            console.onError = window.onerror = function (message, fileName, lineNumber, columnNumber, error) {
                console.send(console.config.levelForJavaScriptErrors,
                    "[" + fileName + ":" + lineNumber + (columnNumber ? ":" + columnNumber : "") + "] " + message +
                        " " + (error ? "(" + error + ")" : "")
                );
            };
        } else {
        ...
        if (console.config.serverUrl && message &&
            ...
            try {
                ...
                if (http) {
                    ...
                    http.send(JSON.stringify({level: level, message: message}));
                }
            } catch (ignored) {
            }
        }
