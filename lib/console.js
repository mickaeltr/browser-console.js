(function () {
    ...
    console.createOrOverrideLogFunction = function (level, originalOrDefaultFunction) {
        return function () {
            var message, returns;
            try {
    ...
    console.createOrOverrideLogFunctions = function () {
        ...
        for (index = 0; index < console.config.levels.length; ++index) {
            ...
            console[level] = console.createOrOverrideLogFunction(level, originalOrDefaultFunction);
        }
    ...
    console.handleJavaScriptErrorsLogging = function () {
        if (console.config.disableJavaScriptErrorsLogging) {
            ...
            console.onError = window.onerror = function (message, fileName, lineNumber, columnNumber, error) {
                console.send(console.config.levelForJavaScriptErrors, message + " " +
                    (error ? " (" + error + ") " : "") + " - " + fileName + ":" + lineNumber +
        ...
        if (console.config.serverUrl && message &&
            ...
            try {
                ...
                if (http) {
                    ...
                    http.send(JSON.stringify({level: level, message: "[" + window.location.href + "] " + message}));
                }
            } catch (ignored) {
            }
        }
